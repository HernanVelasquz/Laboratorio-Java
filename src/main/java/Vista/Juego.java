...
import static Main.Principal.randomNumber;

public class Juego extends javax.swing.JFrame {
    PreguntaDAO Preguntadao = null;
    RespuestasDAO Respuestadao = null;
    PuntajeDAO Puntajedao = null;
    int roundCount = 1;
    int pointsCount = 0;
    String user = "";
    boolean aCorrect;
    ...
    private void ABtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ABtnActionPerformed
        ...
        if (roundCount != 5) {
            if (aCorrect == true) {
                ...
                if (!user.equals("")) {
                    Puntaje newPlayer = new Puntaje(user, pointsCount);
                    Puntajedao.addPointComplete(newPlayer);
                } else {
                    Puntaje newPlayer = new Puntaje(pointsCount);
    ...
    public void setQuestionAndOptions() {
        ...
        Respuestadao = new RespuestasDAO();
        Respuestas ans = Respuestadao.getRespuesta(level);
        String A = ans.getRespuest1();
        String B = ans.getRespuesta2();
        String C = ans.getRespuesta3();
        String D = ans.getRespuesta4();
        if (ans.getAfirmativa1().equals("si")) {
            aCorrect = true;
        } else if (ans.getAfirmativa2().equals("si")) {
            bCorrect = true;
        } else if (ans.getAfirmativa3().equals("si")) {
            cCorrect = true;
        } else {
            dCorrect = true;
        }
