...
import org.sofka.dao.AnswerDAO;
import org.sofka.dao.QuestionDAO;
import org.sofka.dao.ScoreDAO;
...
import static org.sofka.Principal.randomNumber;

public class Game extends javax.swing.JFrame{
    QuestionDAO Preguntadao = null;
    AnswerDAO Respuestadao = null;
    ScoreDAO Puntajedao = null;
    int roundCount = 1;
    int pointsCount = 0;
    String user = "";
    boolean aCorrect;
    ...
    private void ABtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ABtnActionPerformed
        ...
        if (roundCount != 5) {
            if (aCorrect == true) {
                ...
                Puntajedao = new ScoreDAO();
                if (!user.equals("")) {
                    Score newPlayer = new Score(user, pointsCount);
                    Puntajedao.addPointComplete(newPlayer);
                } else {
                    Score newPlayer = new Score(pointsCount);
                    Puntajedao.addPointIncomplete(newPlayer);
                }
                pointsCount = 0;

                org.sofka.view.Score sc = new org.sofka.view.Score();
                this.dispose();
            }
        } else {
            if (aCorrect == true) {
    ...
    public void setQuestionAndOptions() {
        ...
        Preguntadao = new QuestionDAO();
        questionText.setText("\n  " + Preguntadao.getQuestion(level));
        Respuestadao = new AnswerDAO();
        Answer ans = Respuestadao.getRespuesta(level);
        String A = ans.getAsnwerone();
        String B = ans.getAnswertwo();
        String C = ans.getAnswerthree();
        String D = ans.getAnswerfour();
        if (ans.getCorrectanswerone().equals("si")) {
            aCorrect = true;
        } else if (ans.getAnswertwo().equals("si")) {
            bCorrect = true;
        } else if (ans.getAnswerthree().equals("si")) {
            cCorrect = true;
        } else {
            dCorrect = true;
        }
